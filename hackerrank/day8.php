...
while( $line = fgets( $_fp ) ) {
    ...
    if(!empty($pieces[1])){
        $clave = trim($pieces[0], "\n");
        $valor = trim($pieces[1], "\n");
        $phoneBook[$clave]=$valor;
    }else{
        //end of phone book
        $clave = trim($pieces[0], "\n");

        if (array_key_exists($clave, $phoneBook)) {
            echo "$clave=$phoneBook[$clave]";
            echo "\n";
        }else{
            echo "Not found";
            echo "\n";
...
//$phoneBook=Array (["sam"]=>99912222, ["tom"]=>11122222,  ["harry"]=>12299933);
/*$phoneBook=array ("sam"=>99912222, "edward"=>'',  "harry"=>12299933);
